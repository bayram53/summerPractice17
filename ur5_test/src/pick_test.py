...
def check_targets(robot,scene,number,distance):
    ...
    while len(hemme) !=3 :
        ...
        flag = 1     
        for i in hemme:
            if abs(i.pose.position.x - box.pose.position.x) < number  or  abs(i.pose.position.y - box.pose.position.y) < number:
                flag = 0     
        if flag == 0:
            continue
        hemme.append(box)
...
if __name__=='__main__':
    ...
    target_poses = check_targets(robot,scene,3,0.15)
    rospy.sleep(1)
